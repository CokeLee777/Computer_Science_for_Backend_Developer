...
프로그램 실행중에 예기치 않은 상황이 발생할 때 현재 실행중인 작업을 중단하고, 발생된 상황에 대한 우선 처리가 필요함을 CPU에게 알리는 것이다.

지금 수행중인 일보다 더 중요한 일, 예를들면 I/O 입출력, 우선 순위 작업들이 발생하면 먼저 처리하고 하던일로 돌아가서 작업해야 한다.

## Context Switching(문맥 교환)

하나의 프로세스가 CPU를 사용중인 상태에서 다른 프로세스가 CPU를 사용하도록 하기 위해서 이전의 프로세스의 상태(문맥)을 PCB에 보관하고 새로운 프로세스의 상태를 적재하는 것을 말한다. 

보통 인터럽트가 발생하거나, 실행중인 CPU 사용시간을 모두 소모하거나, 입출력을 위해서 대기해야하는 경우에 문맥교환이 일어나게 된다.

### Context Switching의 OverHead란?

일단 오버헤드(OverHead)란 어떤 작업의 처리를 하기위해서 들어가는 간접적인 시간, 메모리 등을 의미한다. 즉, 문맥교환이 일어나는 것 자체가 '오버헤드'라고 할 수 있다. 왜냐하면 문맥교환이 진행되는 순간에는 어떤 유용한 작업을 수행할 수 없기 때문이다.

## IPC(Inter Process Communication)

프로세스는 서로 독립적으로 실행된다. 즉, 독립 되어있다는 것은 서로 다른 프로세스에게 영향을 받지 않는다는 것이다.(스레드는 프로세스 내에서 자원을 공유하므로 독립적이지 못하다.)

이러한 독립적인 상황에서 프로세스간의 통신이 필요한 경우가 있을 것이다. 이를 가능하게 만들어주는 것이 바로 IPC 통신이다.

IPC의 종류는 여러가지가 있는데, 한번 알아보자.

### 익명 PIPE

파이프는 두 개의 프로세스를 연결하는데, 하나의 프로세스는 읽기 작업만 가능하고 나머지 하나는 쓰기만 할 수 있다. 따라서 반이중 통신 방식으로 불린다. 그리고 통신할 두 개의 프로세스를 명확히 알아야 통신이 가능하다.

두개의 파이프의 양방향 통신을 하려면 두 개의 파이프를 연결해야 한다는 단점이 있다. 

이 방식은 매우 간단하게 통신할 수 있다는 장점이 있고, 단순한 데이터의 흐름을 가지는 통신을 할 경우 매우 효과적인 방법이다.

### Named PIPE(FIFO)

익명 PIPE와는 다르게 Named PIPE는 통신할 프로세스들을 서로 모르는 상황에서 사용한다.

Named PIPE 역시 익명 PIPE와 동일하게 반이중 통신 방식이라 양방향 통신을 구현하려면 두 개의 파이프를 사용해야 한다.

### Message Queue

입출력 방식은 Named PIPE와 동일한 방식이다. 다른점은 메시지 큐는 파이프처럼 데이터의 흐름이 아니라 메모리 공간에 담아서 통신한다는 점이다.

메시지 큐는 **브로커 패턴**을 사용하여 동작하는데, 브로커 패턴이란 브로커가 고객과 판매자 사이의 **통신 중재**를 해주는 것이라고 할 수 있다. 메시지 큐는 Producer와 Comsumer가 존재하는데, Producer는 메시지를 큐에 전송하고 Consumer는 큐의 메시지를 처리하는 흐름이라고 할 수 있다.

메시지 큐를 사용하면 **비동기 요청에 대한 처리**를 하고 Queue에 저장하여 Consumer에게 병목을 줄여줄 수 있는데 장단점은 존재한다.

#### 메시지 큐는 왜 사용하는 걸까?

일반적으로 Server-Client 구조를 사용하여 통신을 하면 Client가 Server에게 HTTP 요청을 하고 Server는 그에 대한 응답을 하여 데이터를 전달하는데, 이런 경우 메시지 큐는 필요없다.

그럼 왜 메시지 큐를 사용하는 것일까? 

Server-Cilent 동작원리를 보면 데이터를 송/수신하다가 만약 서버가 갑작스럽게 중단되면 Client는 더 이상 통신을 진행하지 못하게 된다. 메시지 큐를 사용하면 이런 데이터 손실을 막을 수있다.

Producer는 Consumer에게 바로 데이터를 전달하는 것이 아닌, 큐에 데이터를 우선적으로 집어넣고 Consumer에게 전달해주게 된다. 만약 여기서 서버가 중단된다면 서버가 다시 켜질 때까지 큐에 데이터가 담겨져 있으므로 데이터가 손실될 일이 없다. 따라서 메시지 큐를 사용하는 것이다.

### 공유 메모리

공유 메모리는 데이터 자체를 공유하도록 하는 방식의 통신이다. 즉, 스레드처럼 메모리를 공유하여 사용하는 것을 말한다. 프로세스가 공유 메모리 할당을 커널에 요청하면, 커널은 해당 프로세스에 메모리 공간을 할당해주고 이후 모든 프로세스는 해당 메모리 영역에 직접 접근할 수 있게 된다.

이 방식은 중개자 없이 데이터에 바로 접근하므로 IPC 방식중 가장 빠르게 동작한다는 장점이 있다.
...
네트워크 소켓을 사용하여 데이터를 공유하는 방식이다. 클라이언트와 서버가 소켓을 통해서 통신하는 구조로 , 원격에서 프로세스간 데이터를 공유할 때 사용한다.
